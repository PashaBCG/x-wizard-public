...
### Tableau Discovery and Access Checking

For Tableau discovery (URL parsing, Metadata API queries, access rights testing, recommendation engine), see [tableau-setup.md](tableau-setup.md). The discovery script is at `scripts/discover.py`.

**Usage:**
```bash
python3 .cursor/skills/data-advisor/scripts/discover.py "https://tableau.yourcompany.com/#/site/YourSiteName/workbooks/123/datasources"
```

The script accepts any Tableau URL (view, workbook, or datasource), resolves it to the parent workbook, discovers all metadata and datasources, tests access rights, and outputs structured JSON with a recommendation. See [tableau-setup.md](tableau-setup.md) for the full output format and decision tree.
